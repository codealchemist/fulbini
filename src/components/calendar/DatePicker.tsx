...
export function DatePicker({ value, onChange }: { value: Date; onChange: (date: Date) => void }) {
  ...
  return (
    <div className="relative" ref={containerRef}>
      ...
      {open && (
        <>
          {/* On mobile the popover is a fixed, viewport-clamped panel (a
              dropdown anchored to the trigger would overflow off-screen on
              narrow viewports); from sm: up it's a normal anchored dropdown. */}
          <div className="fixed inset-0 z-30 bg-bg/70 sm:hidden" onClick={() => setOpen(false)} />
          <div className="fixed inset-x-4 top-24 z-40 max-h-[80vh] overflow-y-auto rounded-2xl border border-border bg-surface-2 p-3 shadow-2xl sm:absolute sm:inset-x-auto sm:inset-y-auto sm:right-0 sm:top-full sm:z-30 sm:mt-2 sm:w-72 sm:max-h-none sm:overflow-visible">
